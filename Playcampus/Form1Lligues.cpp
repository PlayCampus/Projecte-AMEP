...
namespace CppCLRWinFormsProject {
	...
	System::Void Form1::btnGLMostrarEquips_Click(System::Object^ sender, System::EventArgs^ e) {
		try {
			...
			if (String::IsNullOrWhiteSpace(nomLliga)) {
				MessageBox::Show(Tr(L"No tens cap lliga activa associada."), Tr(L"Av\u00EDs"), MessageBoxButtons::OK, MessageBoxIcon::Warning);
			}
			else {
			...
			MessageBox::Show(Tr(L"Error carregant els equips: ") + ex->Message, Tr(L"Error BD"), MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
	}
	...
	System::Void Form1::btnGLEsborrarEquip_Click(System::Object^ sender, System::EventArgs^ e) {
		try {
			...
			if (equips == nullptr || equips->Rows->Count == 0) {
				MessageBox::Show(Tr(L"No hi ha equips dins de la teva lliga activa."), Tr(L"Av\u00EDs"), MessageBoxButtons::OK, MessageBoxIcon::Information);
			}
			else {
				Form^ finestra = gcnew Form();
				finestra->Text = Tr(L"Esborrar equip de la lliga");
				...
				lblEquip->Text = Tr(L"Selecciona l'equip:");
				...
				btnConfirmar->Text = Tr(L"Esborrar");
				...
				btnCancelar->Text = Tr(L"Cancel\u00B7lar");
				...
				if (finestra->ShowDialog(this) == System::Windows::Forms::DialogResult::OK) {
					...
					System::Windows::Forms::DialogResult confirmacio = MessageBox::Show(
						Tr(L"Vols treure l'equip ") + L"'" + nomEquip + L"'" + Tr(L" de la lliga?"),
						Tr(L"Confirmaci\u00F3"),
					...
					if (confirmacio == System::Windows::Forms::DialogResult::Yes) {
						...
						MessageBox::Show(Tr(L"Equip tret de la lliga correctament."), Tr(L"\u00C8xit"), MessageBoxButtons::OK, MessageBoxIcon::Information);
					}
				}
			}
		}
		catch (Exception^ ex) {
			MessageBox::Show(Tr(L"Error en esborrar l'equip de la lliga: ") + ex->Message, Tr(L"Error BD"), MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
	}

	System::Void Form1::btnGL_EnDesenvolupament_Click(System::Object^ sender, System::EventArgs^ e) {
		MessageBox::Show(Tr(L"Funcionalitat en desenvolupament."));
	}
	...
	System::Void Form1::btnCLGuarda_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		try {
			...
			MessageBox::Show(Tr(L"Lliga creada correctament!"), Tr(L"\u00C8xit"), MessageBoxButtons::OK, MessageBoxIcon::Information);
			...
			MessageBox::Show(Tr(L"Error al crear lliga: ") + ex->Message, Tr(L"Error"), MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
	}

	System::Void Form1::btnGLRetirarTemporada_Click(System::Object^ sender, System::EventArgs^ e) {
		System::Windows::Forms::DialogResult confirmacio = MessageBox::Show(
			Tr(L"Est\u00E0s segur que vols retirar la temporada activa respecte a la teva Lliga?"),
			Tr(L"Confirmaci\u00F3"),
			MessageBoxButtons::YesNo,
			MessageBoxIcon::Warning
		);
		...
		if (confirmacio == System::Windows::Forms::DialogResult::Yes) {
			try {
				...
				MessageBox::Show(Tr(L"La temporada i les seves jornades han estat retirades correctament!"), Tr(L"\u00C8xit"), MessageBoxButtons::OK, MessageBoxIcon::Information);
			}
			catch (Exception^ ex) {
				MessageBox::Show(Tr(L"Error al retirar la temporada: ") + ex->Message, Tr(L"Error BD"), MessageBoxButtons::OK, MessageBoxIcon::Error);
			}
		}
	}
